use std::{cmp::Ordering, fs, io::Cursor, os::unix::fs::PermissionsExt, path::Path};
...
pub(crate) fn write_tree_for(path: &Path) -> anyhow::Result<Option<[u8; 20]>> {
    let mut dir = std::fs::read_dir(path)
        .with_context(|| format!("read directories at {}", path.display()))?;
    ...
    entries.sort_unstable_by(|a, b| {
        // git has specific rules for ordering the entries via comparing name
    ...
    for (entry, file_name, meta) in entries {
        ...
        let mode = if meta.is_dir() {
            ...
            // has at least 1 executable bit set
            "100755"
        } else {
            "100644"
        };
        ...
        let hash = if meta.is_dir() {
            let Some(hash) = write_tree_for(&path)? else {
                // exclude empty directory in parent
                continue;
            };
            hash
        } else {
...
pub(crate) fn invoke() -> anyhow::Result<()> {
    let Some(hash) = write_tree_for(Path::new(".")).context("construct root tree object")? else {
        anyhow::bail!("asked to make tree object for empyt tree")
    };
